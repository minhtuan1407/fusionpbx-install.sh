...
if [ ."$cpu_architecture" = ."x86" ]; then
	wget --http-user=minhtuan1407 --http-password=$switch_token -O /usr/share/keyrings/signalwire-freeswitch-repo.gpg https://freeswitch.signalwire.com/repo/deb/debian-release/signalwire-freeswitch-repo.gpg
...
if [ ."$cpu_architecture" = ."arm" ]; then
	wget --http-user=minhtuan1407 --http-password=$switch_token -O - https://freeswitch.signalwire.com/repo/deb/rpi/debian-release/freeswitch_archive_g0.pub | apt-key add -
	echo "machine freeswitch.signalwire.com login signalwire password $switch_token" > /etc/apt/auth.conf
